...
function formatDataForAI(data) {
    ...
    output += `### 5. AI 解盤執行規範\n`;
    output += `1. **基礎規則**: ${rule}\n`;
    output += `2. **綜合分析**: 請結合日建、月建、月破、旬空與神煞，判斷各爻的旺衰、受沖與感應情況。\n`;
    output += `3. **用神導向**: 針對當前問題，重點分析上述對應的用神爻及其與世爻的生剋關係。\n`;
    output += `4. **最終斷語**: 請提供清晰的吉凶預測與建議，語氣需專業、理性且符合易理。\n\n`;
    
    output += `請根據以上詳盡的排盤數據與執行規範，針對當前問題進行深度的六爻與卦辭分析。`;
    return output;
}
